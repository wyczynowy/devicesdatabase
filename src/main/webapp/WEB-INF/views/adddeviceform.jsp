...
<body>
	...
	<div class="horizontal-center" style="width: 10%; min-width:442px; top: 20; margin: 0 auto;">
		<h1><span class="label label-default">Dodawanie nowego urządzenia</span></h1>
	</div>
	
	<div style="width: 50%; margin: 0 auto; margin-bottom: 50px;">
		<div style="height: 10%; min-height: 550px; margin-top: 5%;">
			<form:form action="${pageContext.request.contextPath}/managedevices/addfilleddeviceform" modelAttribute="newDevice" method="post">
			...
			<div class="form-group">
			  ...
			  <c:forEach var="parkingObject" items="${parkingObjects}">
			    <option <c:if test = "${parkingObject.id == newDevice.parkingObjectId}">selected</c:if> value="${parkingObject.id}">${parkingObject.name}</option>
			    </c:forEach>
			  </select>
			</div>
			 ...
			 <div class="form-group">
 			 ...
 			 <div class="form-group">
			 	...
			 	<input name="manufacturedDate" type="text" value="${convertedManufacteredDate}" id="datepicker" class="form-control" required>
			 </div>
 			 ...
 			 <!-- <div class="form-group">
  			 ...
  			 <div class="form-group">
			 	...
			 	<input name="testedDate" type="text" value="${convertedTestedDate}" id="datepicker2" class="form-control" required>
			 </div>
			...
			<div class="form-group">
			  ...
			  <option <c:if test = "${newDevice.deviceType == 'Server'}">selected</c:if> value="Server">Server</option>
			  <option <c:if test = "${newDevice.deviceType == 'Hub'}">selected</c:if> value="Hub">Hub</option>
			  <option <c:if test = "${newDevice.deviceType == 'Tablica'}">selected</c:if> value="Tablica">Tablica</option>
			  <option <c:if test = "${newDevice.deviceType == 'Czujnik'}">selected</c:if> value="Czujnik">Czujnik</option>
			  <option <c:if test = "${newDevice.deviceType == 'Relay'}">selected</c:if> value="Relay">Relay</option>
			  </select>
			</div>
			 
			 <div class="form-group">
				 <label for="comment">Numer seryjny urządzenia:</label> 
				 <form:textarea path="serial" id="serial" class="form-control" rows="1" required="required"></form:textarea>
				 <!--<c:if test="${pageContext.request.method=='POST'}"><form:errors path="${newnote.adviceDescription}" /></c:if>-->
			 </div>
			 			
			 <input class="btn btn-warning" name="action" type="submit" value = "Zapisz" />
			 <input class="btn btn-warning" name="action" type="submit" value = "Zapisz i dodaj kolejny" />
			 <input class="btn btn-success" type="button" value = "Wróć" onclick="window.location.href='${pageContext.request.contextPath}/managedevices/'"/>
			</form:form>
		</div>
</div>
</body>
